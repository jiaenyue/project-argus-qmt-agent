...
| FR-008 | 系统需实现多源数据一致性监控，定期比对主数据源（miniQMT）和补充数据源（Tushare）的重叠字段（如收盘价），并报告差异。 | 五级监控架构 | 高 | Grafana仪表盘能实时展示多源一致性比率。 | P4.2 |
...
| FR-010 | 整个数据处理流程需通过Airflow进行自动化调度，支持每日定时执行、失败重试和异常处理分支。 | 增强版Airflow DAG | 高 | Airflow DAG能按`@daily`计划自动运行，任务失败后能自动重试3次。 | P4.1 |
...
| NFR-004 | 数据质量评分（综合完整性、准确性、一致性等）月度平均分不低于95分。 | 质量KPI, 成功指标 | 高 | Grafana质量看板显示的月度质量分持续达标。 | P1.3, P4.2 |
...
| **NFR-Maintainability** | | | | | |
| NFR-007 | 系统需提供集中的日志管理（如ELK），方便快速定位和排查问题。 | 基础设施 | 中 | 所有组件的日志均能被采集到Elasticsearch，并能在Kibana中查询。 | P2.1, P4.x |
| NFR-008 | 系统需提供可视化的数据质量监控仪表盘（Grafana），直观展示核心质量指标和系统健康状况。 | 质量看板设计 | 高 | Grafana看板按设计稿实现，数据实时更新。 | P4.2 |

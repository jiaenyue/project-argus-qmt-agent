...
graph LR
    subgraph EPIC1[Epic 1：核心数据采集与处理]
        direction TB
        A1[1.1 miniQMT行情采集] --> A2[1.2 本地文件队列]
    ...
    subgraph EPIC3[Epic 3：黄金数据发布]
        direction TB
        C1[3.1 Parquet文件存储] --> C2[3.2 Schema版本控制]
        C2 --> C3[3.3 后复权数据交付]
    ...
    subgraph EPIC5[Epic 5：系统韧性与安全]
        direction TB
        E1[5.1 混沌工程测试] --> E2[5.2 Windows凭据管理]
        E1 --> E3[5.3 配额成本监控]
...
> **作为** 一名系统架构师，**我想要** 通过本地文件队列来解耦数据采集与处理层，**以便** 提升系统韧性，支持削峰填谷，并在Windows环境下提供可靠的数据传输。
> **验收标准:** 1. 所有原始数据被写入本地文件队列。 2. 文件队列配置了适当的轮转和清理策略。

> #### **1.3. Bronze层标准化**
> **Prio:** 高 | **Status:** `To Do (Sprint 1)` | **Reqs:** `FR-003`
> **作为** 一名数据工程师，**我想要** 实现一个读取本地文件队列的`Bronze`层处理器，**以便** 对来自不同源头的数据进行统一的格式化、去重和基础类型转换。
> **验收标准:** 1. 正确读取本地文件队列。 2. 日期、股票代码格式统一。 3. 重复记录被剔除。
...
> **作为** 一名DevOps工程师，**我想要** 一个基于本地文件的Tushare配额管理器，**以便** 主动避免因配额耗尽导致核心管道中断。
...
> **作为** 一名数据工程师，**我想要** 最终的Gold层数据以`Parquet`格式存储，**以便** 利用其高效的列式存储和压缩能力，从根本上保障数据的存储效率和查询性能。
...
> **验收标准:** 1. 一致性比率在简单Web界面上实时展示。 2. 当比率持续低于99%时，触发P1级告警。 3. 不一致的数据批次被隔离。
...
> #### **5.2. Windows凭据管理 (原故事5.3)**
> **Prio:** 高 | **Status:** `To Do (Sprint 1)` | **Reqs:** `NFR-006`
> **作为** 一名安全官，**我想要** 所有的API Token都通过Windows凭据管理器进行管理，**以便** 杜绝任何形式的凭证硬编码。
> **验收标准:** 1. CI/CD流程中的安全扫描确保没有任何凭证被提交到代码库。

> #### **5.3. 配额成本监控 (原故事5.4)**
> **Prio:** 中 | **Status:** `Product Backlog`
> **作为** 一名项目经理，**我想要** 在简单Web界面上有一个专门的面板来可视化Tushare积分的消耗趋势和云存储的成本，**以便** 我能主动进行成本管理和预算控制。
> **验收标准:** 1. 简单Web界面展示Tushare配额的每日/每周消耗量。 2. 当成本或配额消耗超过预算的90%时，系统能自动发送告警。
...
1.  **立即启动 (P0 - 紧急):**
    ...
    *   [ ] **5.2 Windows凭据管理:** 安全基础，所有API调用前必须完成。
2.  **高优先级 (P1):**
    *   [ ] **1.1 & 1.2 & 1.3 核心采集链路搭建:** 建立从QMT到本地文件队列再到Bronze层的完整数据流。
    *   [ ] **2.4 配额熔断机制:** 保障Tushare集成的稳定性。
    *   [ ] **3.1 Parquet文件存储搭建:** 为数据发布提供基础存储。
3.  **资源分配计划 (Gantt Chart):**
    ...
    gantt
       ...
       Windows凭证管理    :crit, 2023-11-01, 2d
       本地文件队列部署 :2023-11-01, 3d
       Parquet存储环境  :2023-11-06, 3d
    ```
